    ...
    <div>
      ...
      >
        <q-input
          ...
          class="q-ma-md"
          :rules="[
            val => val && val.length >= 3 || 'O campo nome é obrigatorio e deve ter no minimo 3 caracteres.',
          ]"
        >
        ...
        <q-input
          ...
          class="q-ma-md"
          :rules="[
            val => val && val.length >= 3 || 'O campo profissão é obrigatorio e deve ter no minimo 3 caracteres.'
          ]"
        >
          <template v-slot:prepend>
            <q-icon name="work" />
  ...
  methods: {
      ...
      if(this.user.id){
        User.edit(this.user)
          .then(res => {
            ...
            let result = this.usuarios.findIndex((usuario) => usuario.id === res.data.id);
    ...
    remove(user){
      if(confirm(`Deseja remover o(a) usuário(a) ${user.name} ?`)){
        let arrIndice = this.usuarios.findIndex(usuario => usuario.id == user.id);
